...
export class FunctionLibraryComponent implements OnInit {
  ...
  private convertFunctionDefinitionToNode(def: FunctionDefinition): FunctionNode {
    ...
    // Check if we're editing an existing function (it will have an id property)
    const id = (def as any).id || '';  // Use existing ID if available

    return {
      id, // Use existing ID or empty string for new functions
      name: def.name,
